...
const renderHydrogen = (App) => {
    const handleRequest = async function (rawRequest, options) {
        ...
        const { default: inlineHydrogenConfig } = await Promise.resolve().then(() => __importStar(require(
        // @ts-ignore
        // eslint-disable-next-line node/no-missing-import
        'virtual__hydrogen.config.ts')));
        const { default: hydrogenRoutes } = await Promise.resolve().then(() => __importStar(require(
        // @ts-ignore
        // eslint-disable-next-line node/no-missing-import
        ...
        if (url.pathname === constants_1.EVENT_PATHNAME ||
            ...
            return (0, ServerAnalyticsRoute_server_1.ServerAnalyticsRoute)(request, hydrogenConfig.serverAnalyticsConnectors);
        }
        ...
        if (!isReactHydrationRequest) {
            ...
            if (apiRoute &&
                ...
                const apiResponse = await (0, apiRoutes_1.renderApiRoute)(request, apiRoute, hydrogenConfig.shopify, sessionApi);
        ...
        function prepareForStreaming(flush) {
            ...
            if (flush) {
                ...
                if (rscDidError !== null && rscDidError !== void 0 ? rscDidError : ssrDidError) {
                    ...
                    writable.write(encoder.encode((0, error_1.getErrorMarkup)((rscDidError !== null && rscDidError !== void 0 ? rscDidError : ssrDidError))));
                }
                return true;
            }
        }
...
async function hydrate(url, { App, log, request, response, isStreamable, componentResponse, }) {
    const state = (0, parse_1.parseJSON)(url.searchParams.get('state') || '{}');
...
function buildAppSSR({ App, state, request, response, log }, htmlOptions) {
    ...
    let rscDidError;
    const [rscReadableForFizz, rscReadableForFlight] = (0, streaming_server_1.rscRenderToReadableStream)(AppRSC, {
        onError(e) {
            rscDidError = e;
            log.error(e);
        },
...
async function renderToBufferedString(ReactApp, { log, nonce }) {
    if (__WORKER__) {
        ...
        return (0, streaming_server_1.bufferReadableStream)(ssrReadable.getReader());
    }
    else {
